"""
Unit tests for NaiveBayesModel.store_agg_cv_scores.

This module defines three tests for the store_agg_cv_scores method:
1. Normal case:
   - A reasonably sized training set with varied BUSINESS_NAME strings.
   - Verifies that a non-empty "agg_cv_scores.csv" file is written.
2. Edge case:
...
3. Exception case:
   ...
   - Verifies that calling store_agg_cv_scores raises a ValueError due
...
##
## Here we reuse the same structure as in test_store_raw_cv_scores.py.
## ---------------------------------------------------------------------------
...
def test_store_agg_cv_scores_normal(tmp_path) -> None:
    ...
    calling store_agg_cv_scores() should run 5-fold CV and write a non-empty
    "NaiveBayes/agg_cv_scores.csv" file to the tables directory.
    """
    ...
    nb_model.store_agg_cv_scores()

    # Check that the expected file exists and is non-empty
    agg_scores_path: str = os.path.join(
        nb_model.table_output_path,
        "agg_cv_scores.csv",
    )

    assert os.path.isfile(agg_scores_path)
    assert os.path.getsize(agg_scores_path) > 0
...
def test_store_agg_cv_scores_edge(tmp_path) -> None:
    ...
    training set, calling store_agg_cv_scores() should still successfully run
    5-fold CV and write a non-empty "NaiveBayes/agg_cv_scores.csv" file.
    """
...
def test_store_agg_cv_scores_exception(tmp_path) -> None:
    ...
    store_agg_cv_scores() should raise a ValueError because 5-fold
    ...
    # With an empty training set, cross_validate inside store_agg_cv_scores()
