...
As shown in Table x, we can see a negligible difference between Naive-Bayes and Logistic Regression. But we will choose to move forward with the Naive-Bayes classifier, as it is also a relatively simple model with slighlty faster fitting time.

Table x: Comparisons of mean values of scores and times for all models.
